...
async def telegram_send_message(bot_token: str, chat_id: str, text: str) -> dict[str, Any]:
    ...
    r = await integration_request("POST", url, provider="telegram", json=payload)
    try:
        data = r.json()
    except Exception:
        data = {"raw": r.text[:500]}
...
async def whatsapp_send_text_message(
    ...
    r = await integration_request("POST", url, provider="meta_whatsapp", headers=headers, json=body)
    try:
        data = r.json()
    except Exception:
        data = {"raw": r.text[:800]}
...
async def wechat_mp_get_access_token(app_id: str, app_secret: str) -> tuple[str, int]:
    ...
    r = await integration_request("GET", url, provider="wechat_mp")
    data = r.json()
...
async def wechat_mp_template_send(
    ...
    r = await integration_request("POST", url, provider="wechat_mp", json=body)
    data_out = r.json()
